...
public class MainTest {
	public static class MainAddTest {
		...
		public void testEmptyAdd() {
			int ret = server.add("", "", "");
			assertTrue("Expected -1 if arguments are not well formed.",
					ret == -1);
		}
		...
		public void testHugeAdd() {
			...
			assertFalse("Expected error indication when adding huge message.",
					ret > 0);
		}
		...
		public void testWrongNumberAdd() {
			...
			int ret = server.add("I lolz in da lolinator!", my_number,
					my_number);
			assertTrue("Expecting error number from add.", ret < 0);
		}
	}

	public static class MainDeleteTest {
		static MainServer server;
		...
		public void testDelete() {
			...
			assertTrue("Expected response from delete to be key from add.",
					add_key == del_key);
		}
	...
	public static class MainReplaceTest {
		static MainServer server;
		...
		public void testReplace() {
			...
			assertTrue("Expecting integer above 0 on successful replace.",
					rep_key > 0);
		}
		...
		public void testEmptyReplace() {
			...
			assertTrue("Expecting -1 on replace with non well formed message.",
					rep_key == -1);
		}
	...
	public static class MainFetchTest {
		static MainServer server;
	...
	public static class MainFetchCompleteTest {
		static MainServer server;
		...
		public void testFetchComplete() {
			...
			assertTrue(
					"Expecting positive integer for successful fetch completion.",
					res > 0);
		}
